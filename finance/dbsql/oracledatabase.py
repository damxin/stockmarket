...
#            print("oracle continfo(%s)"% cntinfo)
    ...
    def execSelectManySql(self,strsql):
        ...
        strmanysql = "select %s from ( select rownum rn, %s where rownum <= %d) a where rn > %d" % (strsql[6:endcol],strsql[6:], self.fetchmanystartrow+DataBase.ROWNUM, self.fetchmanystartrow)
#        print(strmanysql)
